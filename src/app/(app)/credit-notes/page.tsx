...
interface CreditNote { id: string; credit_note_code: string; customer_name: string; invoice_code: string; credit_date: string; grand_total: number; status: string; reason: string; }
...
export default function CreditNotesPage() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/credit-notes?${p}`).then((r) => r.json()).then(setNotes).finally(() => setLoading(false));
  }, [search, status]);
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">Issued to customers for returns or adjustments</p>
        </div>
        <Link href="/credit-notes/new" className="inline-flex items-center gap-2 bg-teal-700 text-white px-4 py-2.5 rounded-lg hover:bg-teal-800 transition-colors text-sm font-medium shadow-sm">
      ...
      <div className="flex gap-3 mb-5">
        <div className="flex-1 relative">
          ...
          <input type="text" placeholder="Search credit notes..." value={search} onChange={(e) => setSearch(e.target.value)} className="w-full pl-10 pr-4 py-2 border border-gray-200 rounded-lg text-sm bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-600/20 focus:border-teal-600 transition-all" />
        </div>
            ...
            <tbody>
              {notes.map((n) => (
                <tr key={n.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                  <td className="px-5 py-3 text-sm font-medium text-teal-700">{n.credit_note_code}</td>
                  <td className="px-5 py-3 text-sm text-gray-700">{n.customer_name}</td>
                  <td className="px-5 py-3 text-sm text-gray-500">{n.invoice_code || "-"}</td>
                  <td className="px-5 py-3 text-sm text-gray-500 tabular-nums">{n.credit_date}</td>
                  ...
                  <td className="px-5 py-3"><Link href={`/credit-notes/${n.id}`} className="text-sm text-teal-600 hover:text-teal-800">View</Link></td>
                </tr>
              ))}
              {notes.length === 0 && <tr><td colSpan={8} className="px-5 py-10 text-center text-gray-400 text-sm">No credit notes found</td></tr>}
            </tbody>
          </table>
        )}
      </div>
