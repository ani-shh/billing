...
interface Invoice { id: string; invoice_code: string; customer_id: string; }
...
const REASONS = ["Goods Returned", "Pricing Error", "Damaged Goods", "Quantity Dispute", "Service Not Rendered", "Other"];
...
export default function NewCreditNotePage() {
  ...
  const handleSave = async () => {
    if (!customerId || items.length === 0) { alert("Select a customer and add items"); return; }
    setSaving(true);
    const res = await fetch("/api/credit-notes", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ customer_id: customerId, invoice_id: invoiceId || undefined, credit_date: creditDate, reference_no: referenceNo, reason, notes, subtotal, tax_total: taxTotal, grand_total: grandTotal, items: items.map(({ id: _, ...rest }) => rest) }) });
    if (res.ok) router.push("/credit-notes"); else alert("Failed");
    setSaving(false);
  };
  ...
  return (
    <div className="max-w-5xl">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-xl font-semibold text-gray-900">New Credit Note</h1>
        <div className="flex gap-3">
          ...
          <Link href="/credit-notes" className="inline-flex items-center justify-center rounded-lg border border-gray-200 bg-white p-2.5 text-gray-400 hover:bg-gray-50">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-5 overflow-visible">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Customer <span className="text-red-500">*</span></label>
              <SearchSelect value={customerId} onChange={setCustomerId} placeholder="Select Customer" options={customers.map((c) => ({ value: c.id, label: c.name }))} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Against Invoice</label>
              <SearchSelect value={invoiceId} onChange={setInvoiceId} placeholder="Select Invoice (optional)" options={invoices.filter((i) => !customerId || i.customer_id === customerId).map((i) => ({ value: i.id, label: i.invoice_code }))} /></div>
            <div><label className="block text-sm font-medium text-gray-700 mb-1">Credit Date <span className="text-red-500">*</span></label>
              <input type="date" value={creditDate} onChange={(e) => setCreditDate(e.target.value)} className={ic} /></div>
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-5">
          ...
          {items.length > 0 && (
            <div className="mt-3 flex justify-end"><div className="w-64 space-y-1.5 text-sm">
              ...
              <div className="flex justify-between border-t border-gray-200 pt-2 font-semibold text-gray-900"><span>Total Credit</span><span className="tabular-nums">{fmt(grandTotal)}</span></div>
            </div></div>
          )}
        </div>
